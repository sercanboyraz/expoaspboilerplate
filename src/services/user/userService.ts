...
class UserService {
  public async create(createUserInput: GetUserOutput) {
    let result = await http.post('api/services/app/User/Create', createUserInput);
    return result.data.result;
  }

  public async update(updateUserInput: GetUserOutput) {
  ...
  public async getAll(pagedFilterAndSortedRequest: PagedUserResultRequestDto): Promise<PagedResultDto<GetUserOutput>> {
    let result = await http.get('api/services/app/User/GetAll', { params: pagedFilterAndSortedRequest });
    return result.data.result;
  }
  public async getAllBaseUserInfo(filter: BaseUserDto) {
    let result = await http.post('api/services/app/User/BaseUserInfo', filter);
    return result.data.result;
  }
}
